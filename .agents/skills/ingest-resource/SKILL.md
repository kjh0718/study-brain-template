...
7. 강의와 연결할 때 `Lecture.resources[]`와 `Resource.lectures` **양쪽을 함께** 고친다. 한 LEC 안에서 같은 RES는 한 항목만 두고 구간은 그 항목의 `pages`에 모은다.
...
- 목차만으로 수업 진도나 사용 페이지를 추정하지 않는다. 사용자가 밝힌 근거 없이 다른 LEC과 연결하지 않는다.
- 생략된 `pages`를 자료 전체로 해석하지 않는다. 확인하지 못한 범위라는 뜻이다.
