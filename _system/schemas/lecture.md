...
- LEC-YYYYMMDD-NN 형식의 날짜는 수업일을 사용한다. 수업일이 미확인이면 생성일을 사용하고 date는 null로 남긴다.
...
- 서로 충돌하는 사실은 근거를 함께 남기며, 명시적 변경인지 판단하는 처리는 향후 L5에 따른다.
...
이 파일은 데이터 규격이다. 실제 전사 처리는 향후 SECOND-BRAIN.md의 L1이 담당하며 Course 대시보드, 관련 Resource, Cluster와 _system/log.md 갱신을 포함한다. 이 스키마 작성만으로 자동 전사나 ingest가 구현되는 것은 아니다.
