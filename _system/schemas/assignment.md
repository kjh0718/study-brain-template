...
권장 형태는 `ASM-<과목 slug>-<날짜>-<NN>`이다. 예: `ASM-general-physics-2-20260908-01`. 날짜는 과제가 부여된 날이며 확인되지 않으면 노트 생성일을 쓴다. 같은 과목·같은 날 여러 과제가 나올 수 있으므로 일련번호를 쓰되, 번호는 (ASM, 과목, 날짜) 범위에서만 세고 저장소 전체에서 충돌을 확인한다.
