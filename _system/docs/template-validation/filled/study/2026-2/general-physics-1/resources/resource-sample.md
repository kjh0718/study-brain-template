---
schema: 1
type: resource
id: RES-general-physics-1-week03-slides
title: Chapter 3 - Momentum
status: active
topics: 
  - momentum
related: []
created: 2026-09-08
updated: 2026-09-08
course: CRS-20260908-01
resource_type: slides
authority: professor
source: raw/2026-2/general-physics-1/resources/week03-slides.pdf
lectures: 
  - LEC-20260908-01
---

<!-- id: RES-<과목 slug>-<자료 slug> 형태. 등록일을 넣지 않는다. 예: RES-general-physics-2-ch03-slides -->
<!-- course: 자료를 처음 등록한 과목(canonical home)의 CRS ID. null을 쓰지 않는다. 다른 과목에서 재사용하면 복제하지 않고 related에 그 과목의 CRS ID를 적는다. -->
<!-- resource_type: slides, textbook, handout, paper, article, practice, lab, video, dataset, other -->
<!-- authority: professor, official-lms, textbook, student-provided, external, ai-generated, unknown -->
<!-- source: 로컬 원본은 raw/<term>/<course-slug>/resources/ 아래 경로, 외부 자료는 접근 가능한 URL. 외부 URL을 굳이 내려받지 않는다. -->
<!-- 선택 필드: 페이지 수를 확인했으면 page_count를 추가한다. -->

## Overview

## Structure

| 구간 | 내용 |
|---|---|
|  |  |

## Key Concepts

<!-- 개념 후보를 여기 적는다. CON 생성 여부는 L4가 판단한다. -->

-

## Material Emphasis

<!-- 자료 자체의 표시(굵게·별표·요약 슬라이드). 페이지를 함께 적는다. -->

-

## Professor Emphasis

<!-- Lecture 원문으로 확인된 발언만. 해당 LEC과 원문 위치를 함께 적는다. -->

-

## Lecture Usage

<!-- 사용 페이지의 기준 데이터는 Lecture.resources[].pages다. 여기에는 그 요약만 둔다. -->
<!-- 여러 Lecture가 겹치는 페이지를 써도 정상이다. 범위를 확인하지 못한 Lecture는 pages 칸을 미확인으로 둔다. -->

-

## Exam References

-

## Assignment References

-

## Version / Provenance

- 판·버전:
- 배포 경로:
- 작성자:

## My Notes

<!-- AI-PROTECTED -->

## Source

-
