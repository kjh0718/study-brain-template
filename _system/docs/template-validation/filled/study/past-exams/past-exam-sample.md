---
schema: 1
type: past-exam
id: PEX-20260908-01
title: 물리학 2024 중간고사 기출
status: needs-review
topics: []
related: []
created: 2026-09-08
updated: 2026-09-08
course: CRS-20260908-01
year: 2024
exam_type: midterm
source: https://example.edu/past-exams/2024-midterm.pdf
authority: student-provided
provenance: reconstructed
concepts: 
  - CON-20260908-01
---

<!-- course: 이 기출을 현재 사용하는 CRS ID. null을 쓰지 않는다. 과목이 미확정이면 노트를 만들지 않고 입력을 inbox/에 남긴다. -->
<!-- year: 확인된 시험 연도 정수만 넣는다. 모르면 null을 유지한다. 실제 시행 학기는 새 필드 없이 ID와 본문 자료 식별 / 출처 절에 적는다. -->
<!-- exam_type: midterm, final, quiz, practical, other, unknown -->
<!-- source: 로컬 원본은 raw/<term>/<course-slug>/past-exams/ 아래 경로, 외부 자료는 접근 가능한 참조. <term>은 이 기출을 사용하는 CRS의 학기다. -->
<!-- authority: professor, official-lms, textbook, student-provided, external, ai-generated, unknown -->
<!-- provenance: official(원본 시험지), reconstructed(학생 복원본), unknown -->
<!-- 질문 연결에는 questions 필드가 없다. related와 본문을 사용한다. -->

## 자료 식별 / 출처

- 접근 여부: 확인하지 못함. 링크만 등록했고 문항 분석은 보류한다.
- 배포 경로: 학생 공유
- 표본 범위: 1회차

## 문항별 분석

| 번호 | 유형 | 주제 | 관련 Concept | 배점 |
|---|---|---|---|---|
|  |  |  |  |  |

<!-- 원본에 번호가 없으면 로컬 표기를 쓰고 원본 번호가 아님을 명시한다. -->

## 제공된 정답 / 해설

<!-- 자료에 실제로 실려 있던 정답과 해설만. -->

-

## AI 풀이 / 검증 필요

<!-- 위 절과 반드시 분리한다. analyzed는 공식 정답 검증 완료를 뜻하지 않는다. -->

-

## Concept 연결

-

## 출제 경향의 근거

<!-- 한 회차만으로 경향을 확정하지 않는다. 표본 수와 출처 범위를 함께 적는다. -->

-

## My Notes

<!-- AI-PROTECTED -->

## Source

-
